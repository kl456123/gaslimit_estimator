...
export function getRangoContract(chain: Chain, provider: JsonRpcProvider) {
  ...
  const rangoContract = new ethers.Contract(rangoAddress, abis, provider);
  return rangoContract;
}

export function getLifiContract(chain: Chain, provider: JsonRpcProvider) {
  const abis = [
...
export function getoneInchContract(chain: Chain, provider: JsonRpcProvider) {
  const abis = [""];
